...
boolean touchpad_error = LOW; // sent high if touch pad routine times out
//
...
void setup()
{
  ...
  if (watchdog_caused_reboot()) {
    touchpad_error = 1; // tp has errors so don't use it
  } 
  else {
    ...
    touchpad_error = 0; // tp initialized (didn't hang)
  }
//
}
...
// ************************************Main Loop***************************************************************
void loop() {
  if (touchpad_error == LOW) { // check if touchpad is OK
    digitalWrite(TP_ERROR, LOW); // drive I/O pin low when tp is responding normally
//
...
//
    ...
    digitalWrite(TP_ERROR, HIGH); // drive I/O pin high to show the tp has problems 
  }
//
