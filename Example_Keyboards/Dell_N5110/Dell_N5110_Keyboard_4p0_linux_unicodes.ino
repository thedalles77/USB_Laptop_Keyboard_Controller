        ...
        if (!digitalRead(Col_IO[y]) && (old_key[x][y]) && (!slots_full)) { // check if key pressed and not previously pressed and slots not full
          ...
          if (Fn_pressed) {  // Fn_pressed is active low so it is not pressed and normal key needs to be sent
            ...
            if ((normal[x][y] == KEY_SCROLL_LOCK) && (mod_alt_r == MODIFIERKEY_RIGHT_ALT)) {
              // first release the right alt key so the unicode will work
              clear_mod(MODIFIERKEY_RIGHT_ALT); // function loads 0 into the mod_alt_r variable
              ...
              //  *******Linux Unicode sequence to give a "<" is Control Shift u 3 c Enter*****************************
              ...
              delay(5);
              Keyboard.press(KEY_3); // Push 3
              delay(5); 
              Keyboard.release(KEY_3); // Release 3
              delay(5); 
              ...
              delay(5);
              // put right alt back to being set since the key is still being pushed
              load_mod(MODIFIERKEY_RIGHT_ALT); // function loads the mod_alt_r variable
              send_mod(); // function sends all mod's over usb including the right alt
            }
            // ****** If Í and right shift are pressed, send capital i acute "Í" ******
            else if ((normal[x][y] == KEY_SCROLL_LOCK) && (mod_shift_r == MODIFIERKEY_RIGHT_SHIFT)) {
              // first release the right shift key so the unicode will work
              clear_mod(MODIFIERKEY_RIGHT_SHIFT); // function loads 0 into the mod_shift_r variable
              send_mod(); // function sends all mod variable's over usb
              delay(5); // delay 5 milliseconds to make sure it gets sent over USB before next character
              //  *******Linux Unicode sequence to give a Í is Control Shift u c d Enter*****************************
              Keyboard.press(MODIFIERKEY_LEFT_CTRL); // Push and hold Left Control
              delay(5);
              Keyboard.press(MODIFIERKEY_LEFT_SHIFT); // Push and hold Left Shift
              delay(5);
              ...
              Keyboard.release(KEY_C); // Release c
              delay(5); 
              Keyboard.press(KEY_D); // Push d
              delay(5); 
              Keyboard.release(KEY_D); // Release d
              delay(5);
              Keyboard.press(KEY_ENTER); // Push Enter
              delay(5); 
              Keyboard.release(KEY_ENTER); // Release Enter
              ...
              load_mod(MODIFIERKEY_RIGHT_SHIFT); // function loads the mod_shift_r variable
              send_mod(); // function sends all mod's over usb including the right shift
            }
            // ****** If Í is pressed (without right shift or right alt), send lower case i acute "í" ******
            else if (normal[x][y] == KEY_SCROLL_LOCK) {
              //  *******Linux Unicode sequence to give a í is Control Shift u e d Enter*****************************
              Keyboard.press(MODIFIERKEY_LEFT_CTRL); // Push and hold Left Control
              delay(5);
              Keyboard.press(MODIFIERKEY_LEFT_SHIFT); // Push and hold Left Shift
              delay(5);
              ...
              Keyboard.release(MODIFIERKEY_LEFT_SHIFT); // Release Left Shift
              delay(5);
              Keyboard.press(KEY_E); // Push e
              delay(5); 
              Keyboard.release(KEY_E); // Release e
              delay(5); 
              Keyboard.press(KEY_D); // Push d
              delay(5); 
              Keyboard.release(KEY_D); // Release d
