...
# Open an editor with a text file that lists all the keyboard keys. The program will send over USB, the two
# GPIO pin numbers that are connected when a key is pressed.
...
# List the WeAct I/O pins in the array that are connected to the keyboard FPC cable. I/O's 0, 23, 25, 43 and up are not used.
# If the program finds that 2 pins are always connected (probably grounds or LEDs), remove them from the I_O list and
# comment them in the Send_GP function. 

I_O = [board.GP1, board.GP2, board.GP3, board.GP4, board.GP5, board.GP6, board.GP7, board.GP8,  
board.GP9, board.GP10, board.GP11, board.GP12, board.GP13, board.GP14, board.GP15, board.GP16, board.GP17, 
board.GP18, board.GP19, board.GP20, board.GP21, board.GP22, board.GP24, board.GP26,
board.GP27, board.GP28, board.GP29, board.GP30, board.GP31, board.GP32, board.GP33, board.GP34, board.GP35, board.GP36, 
board.GP37, board.GP38, board.GP39, board.GP40, board.GP41, board.GP42]
# This function converts the board.GP number from the array into a text number and sends it over USB.
# It will work with up to 42 GPIO pins (for the WeAct RP2350B)
def Send_GP(GP):
    if GP == board.GP1:
        layout.write('1')
    elif GP == board.GP2:
