...
//
// Revision History
// Rev New - Dec 1, 2025 - Original Release
// Rev A - March 27, 2026 - Added scroll with trackpoint+middle mouse button
// Keyboard LEDs
#define LED_PWR 25    // Anode side of LEDs always driven high to 3.3 volts
#define CAPS_LED 38   // Active low signal to Caps lock LED cathode (with 100 ohm dropping resistor) 
//#define NUM_LED TBD    // Active low signal to Num lock LED cathode (with 100 ohm dropping resistor)
#define BLINK_LED 13   // The LED on the Teensy is programmed to blink as a heartbeat
#define BACKLIGHT 37   // The backlight PWM control input pin
...
#define TP_CLK 40  // Trackpoint ps/2 clock connected to Teensy I/O pin
#define TP_RESET 36  // Trackpoint active high reset 
...
// 
int brightness = 0;   // Variable to store the PWM duty cycle (0=off, 255=max) for the keyboard backlight brightness
//
...
unsigned char mstat; // trackpoint status 8 bit register = Y overflow, X overflow, Y sign bit, X sign bit, Always 1, Middle Btn, Right Btn, Left Btn
signed char mx; // trackpoint x movement = 8 data bits. The sign bit is in the status register to 
           // make a 9 bit 2's complement value. Left to right on the trackpoint gives a positive value. 
signed char my; // trackpoint y movement also 8 bits plus sign bit in status reg. trackpoint movement toward the display gives a positive value.
//
...
// Determine if left or right trackpoint buttons have changed since last polling cycle using xor
...
// Don't send button status if there's no change since last time. 
  if (button_change){
    Mouse.set_buttons(left_button, 0, right_button); // send left, 0, and right button status
  }
...
//
// *******keyboard LEDs
// Turn on or off the Caps Lock LED based on bit 1 from the keyboard_leds 
