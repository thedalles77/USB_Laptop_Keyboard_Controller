...
// Rev A Dec 29, 2021 - Added medium and fast speed movement to Ergotrac
//
// keyboard values
...
int noise_zone = 10; // dead zone around the center. 
char slowdown_left = 70; // factor to slow the curser movement down going left
char slowdown_right = 70; // factor to slow the curser movement down going right
char slowdown_up = 70; // factor to slow the curser movement down going up
char slowdown_down = 70; // factor to slow the curser movement down going down
...
// All the other normal keys are sent
            ...
            noise_zone = 10; // Set noise zone and slowdowns to default values
            slowdown_left = 70; // movement left needs more slowdown for some reason
            slowdown_right = 70;
            slowdown_up = 70;
            slowdown_down = 70;
          }
            ...
            if (noise_zone >= 1000) { // toggle back to normal or huge (off) value
              noise_zone = 10; // back to start up value
            }
            else {
...
// Figure out if the X value is to the right or the left of the center resting value
    if (x_read > x_center) {  // is X ADC to the right of the resting position?
      if ((x_read - x_center) >= 525) {
        x_delta = (x_read - x_center) / (slowdown_right / 4); // fast right movement
      }
      else if ((x_read - x_center) >= 375) {
        x_delta = (x_read - x_center) / (slowdown_right / 2); // medium right movement
      }
      else {
        x_delta = (x_read - x_center) / slowdown_right; // slow right movement
      }
    }
    else { // X ADC is equal or to the left of the resting position
      if ((x_center - x_read) >= 525) { 
        x_delta = ((x_center - x_read) / (slowdown_left / 4)) * -1; // fast left movement. Answer should be negative so multiply by -1
      }
      else if ((x_center - x_read) >= 375) { 
        x_delta = ((x_center - x_read) / (slowdown_left / 2)) * -1; // medium left movement. Answer should be negative so multiply by -1  
      }
      else {  
        x_delta = ((x_center - x_read) / slowdown_left) * -1; // slow left movement. Answer should be negative so multiply by -1
      }
    }
...
// Figure out if the Y value is above or below the center resting value
    if (y_read > y_center) { // is Y ADC above the resting position?
      if ((y_read - y_center) >= 525) {
        y_delta = ((y_read - y_center) / (slowdown_down / 4)) * -1; // fast down movement
      }
      else if ((y_read - y_center) >= 375) {
        y_delta = ((y_read - y_center) / (slowdown_down / 2)) * -1; // medium down movement
      }
      else {
        y_delta = ((y_read - y_center) / slowdown_down) * -1; // slow down movement
      }
    }
    else { // Y ADC is equal or above the resting position
      if ((y_center - y_read) >= 525) { 
        y_delta = (y_center - y_read) / (slowdown_up / 4); // fast left movement. Answer should be negative so multiply by -1
      }
      else if ((y_center - y_read) >= 375) { 
        y_delta = (y_center - y_read) / (slowdown_up / 2); // medium left movement. Answer should be negative so multiply by -1  
      }
      else {  
        y_delta = (y_center - y_read) / slowdown_up; // slow left movement. Answer should be negative so multiply by -1
      }
    }
  } 
...
// send the x and y data over usb if either one is not at center position
  ...
  digitalWrite(ErgoTrac_Ref, LOW); // return reference pulse to 0V
