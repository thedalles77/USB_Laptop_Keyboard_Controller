...
// Force a clean USB serial identification 
  Serial.begin(115200);
  delay(500); // Short buffer to let the hardware state settle

  // Initialize the standard HID Keyboard layout
  Keyboard.begin();
}

void loop() {
  // Single Pass Matrix Scan drives each row low, one at a time and reads all the columns with pull ups inside RP2040
  for (int r = 0; r < NUM_ROWS; r++) {
    pinMode(rowPins[r], OUTPUT); // make the selected row pin an output (it was floating as an input)
      ...
      if (currentPressed != lastKeyState[r][c]) { // check if current key state is not the same as the last loop
        ...
        if (standardKey != 0) { // non zero key code in the array is selected
          if (currentPressed) { // high indicates the key was just pressed
            Keyboard.press(standardKey); // send the key press
          } else { // key was just released
            Keyboard.release(standardKey); // send the key release
          }
        }
