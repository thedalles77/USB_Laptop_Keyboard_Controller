...
# Define the 20 remaining native Pico GPIO pins used to scan the keyboard. 
# GP2 & GP3 are programmed for I2C which causes GP6, GP7, GP14, and GP15 to 
# not be available due to internal hardware conflicts with the active I2C1 block.
pico_pins = [
    board.GP0, board.GP1, board.GP4, board.GP5,
...
def set_pin_active_low(pin_index):
    """
    Configures a single target pin out of the 36 possibilities to drive LOW.
    Indices 0-19 are native Pico pins. Indices 20-35 are MCP23018 pins.
    """
    if pin_index < 20:
...
print("Starting 36-Pin Matrix Scanner...")
...
while True:
    # Cycle sequentially through all 36 physical connections
    for drive_pin in range(36):
        set_pin_active_low(drive_pin)
