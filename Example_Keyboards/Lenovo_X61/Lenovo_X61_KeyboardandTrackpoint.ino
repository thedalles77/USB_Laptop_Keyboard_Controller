...
// Rev A - March 27, 2026 - Added scroll when middle mouse button is pressed with TP pressure up or down.
// Trackpoint signals
...
//
// Row con pin # 32,28,03,20,16,12,08,04,22,26,30,18,14,10,06,02,24
...
//
// Column con pin # 05,09,13,17,15,11,07,03
// Teensy I/O     # 07,06,04,23,22,05,19,08
...
unsigned char mstat_track; // trackpoint status reg = Y overflow, X overflow, Y sign bit, X sign bit, Always 1, Middle Btn, Right Btn, Left Btn
signed char mx_track; // trackpoint x movement = 8 data bits. The sign bit is in the status register to 
         // make a 9 bit 2's complement value. Left to right on the trackpoint gives a positive value. 
signed char my_track; // trackpoint y movement also 8 bits plus sign. trackpoint movement away from the user gives a positive value.
...
// scroll if middle button is pressed and my_track is non-zero
    if ((0x04 & mstat_track) == 0x04) { // check for middle button press 
      if (my_track >= 35) { // check if trackpoint is being pushed hard up
        Mouse.scroll(-4); // scroll the cursor fast
        delay(200);
      }
      else if (my_track >= 3) { // check if trackpoint is being pushed lightly up
        Mouse.scroll(-1); // scroll the cursor slow
        delay(200);
      }
      else if (my_track <= -35) { // check if trackpoint is being pushed hard up
        Mouse.scroll(4); // scroll the cursor fast
        delay(200);
      }
      else if (my_track <= -3) { // check if trackpoint is being pushed lightly down
        Mouse.scroll(1); // scroll the cursor slow
        delay(200);
      }   
    } 
// if no scroll, send the x and y trackpoint data over USB if x or y is non-zero
    else if ((mx_track != 0x00) || (my_track != 0x00)) {
...
// Determine if left or right trackpoint buttons have changed since last polling cycle
    button_change_track = ((left_button_track ^ old_left_button_track) | (right_button_track ^ old_right_button_track));
//
// Send trackpoint button status if there was a change since last time. 
    if (button_change_track) {
      Mouse.set_buttons(left_button_track, 0, right_button_track); // send left/right button status (middle is always zero)
